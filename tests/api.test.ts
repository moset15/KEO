...
function environment(overrides: Partial<Env> = {}): Env {
  return {
    ASSETS: {
      fetch: vi
        .fn<Env["ASSETS"]["fetch"]>()
        .mockResolvedValue(new Response("Static page")),
    },
    ...overrides,
  };
}
...
function request(
  body: unknown = { query: "When is the next election?", mode: "ask" },
): Request {
...
describe("HTTP API boundary", () => {
  it("rate-limits curated requests even with paid AI disabled", async () => {
  ...
  it("does not call the provider when credentials exist but the enable switch is off", async () => {
    const fetcher = vi.fn();
    ...
    expect(await response.text()).toContain('"mode":"curated"');
  ...
  it.each(["GET", "PUT", "DELETE"])(
    "rejects %s investigation requests",
    async (method) => {
      const response = await handle(
        new Request(origin + "/api/investigate", { method }),
        environment(),
      );
  ...
  it("bounds actual body bytes even without a Content-Length header", async () => {
    ...
    const accepted = new Request(origin + "/api/investigate", {
      method: "POST",
      body: "ééé",
    });
...
describe("investigation stream and rate limiting", () => {
  it("streams a curated result with a trace and honest uncertainty without network access", async () => {
    ...
    expect(lines[0]).toMatchObject({
      type: "step",
      message: "Searching the curated evidence library",
    });
    ...
    expect(result.evidence_ids).toEqual(
      result.evidence.map((item: { id: string }) => item.id),
    );
  ...
  it("limits live requests using the connecting IP before contacting the gateway", async () => {
    const limit = vi
      .fn<NonNullable<Env["RATE_LIMITER"]>["limit"]>()
    ...
    const response = await handle(
      input,
      environment({ ...liveConfig, RATE_LIMITER: { limit } }),
    );
    expect(response.status).toBe(429);
  ...
  it("returns an unavailable result when an allowed live request fails upstream", async () => {
    ...
    expect(lines.at(-1)).toMatchObject({
      type: "result",
      result: { mode: "unavailable", status: "insufficient_evidence" },
    });
