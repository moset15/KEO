...
function analysis(overrides: Partial<Analysis> = {}): Analysis {
  return {
    ...
    confidence: "high",
    summary:
      "The cited constitutional article explains the regular election schedule.",
    observations: ["The record is legal background."],
    evidence_ids: [curatedEvidence[0].id],
    contradictions: [],
    information_gaps: [
      "This is not confirmation of a current administrative notice.",
    ],
...
describe("Cloudflare AI Gateway requests", () => {
  it("never enables paid calls just because credentials exist", () => {
    expect(gatewayReady({ ...config, AI_ENABLED: undefined })).toBe(false);
    expect(gatewayReady({ ...config, AI_ENABLED: "false" })).toBe(false);
    expect(gatewayReady({ ...config, AI_ENABLED: "TRUE" })).toBe(false);
  });
  ...
  it("uses the configured gateway and model without storing, logging or caching claims", async () => {
    const fetcher = vi
      .fn<typeof fetch>()
      .mockResolvedValue(Response.json(message("Complete")));
    await new GatewayProvider(config, fetcher).response({
      input: "Public question",
    });
    const [url, init] = fetcher.mock.calls[0];
    expect(url).toBe(
      `https://gateway.ai.cloudflare.com/v1/${config.AI_GATEWAY_ACCOUNT_ID}/keo-tests/openai/responses`,
    );
  ...
  it.each([
    ...
    "rejects incomplete or unsafe configuration before fetching",
    async (invalid) => {
      const fetcher = vi.fn<typeof fetch>();
      await expect(
        new GatewayProvider(invalid, fetcher).response({}),
      ).rejects.toThrow();
  ...
  it("rejects upstream HTTP errors and malformed response envelopes", async () => {
    const failed = vi
      .fn<typeof fetch>()
      .mockResolvedValue(
        new Response("Internal provider detail", { status: 502 }),
      );
    await expect(
      new GatewayProvider(config, failed).response({}),
    ).rejects.toThrow("Gateway request failed (502)");
    const malformed = vi
      .fn<typeof fetch>()
      .mockResolvedValue(Response.json({ output: "invalid" }));
    await expect(
      new GatewayProvider(config, malformed).response({}),
    ).rejects.toThrow();
  });
  ...
  it("requires web search and requests a strict JSON schema before accepting cited results", async () => {
    ...
    expect(search.tools[0].filters.allowed_domains).not.toContain(
    ...
    expect(JSON.parse(request.input).evidence[0].id).toBe(
      curatedEvidence[0].id,
    );
    expect(result.evidence).toEqual([curatedEvidence[0]]);
  ...
  it.each([
    ...
    JSON.stringify(
      analysis({ status: "invented-status" as Analysis["status"] }),
    ),
  ...
  it("rejects a search that has not completed before asking for a verdict", async () => {
    ...
    ).rejects.toThrow("Search did not complete");
  ...
  it("downgrades an uncited confident answer to insufficient evidence", async () => {
    const { provider } = mockedProvider(
      searchResponse(),
      message(JSON.stringify(analysis({ evidence_ids: [] }))),
    );
    const result = await provider.investigate(
      "Is this notice real?",
      [],
...
describe("retrieved citation integrity", () => {
  it("accepts declared source hosts and real subdomains", () => {
    expect(sourceForUrl("https://www.iebc.or.ke/notices/123")?.source_id).toBe(
      "iebc",
    );
    expect(sourceForUrl("https://notices.iebc.or.ke/123")?.source_id).toBe(
      "iebc",
    );
  });
  ...
  it("uses actual citations, filters unsuitable URLs, deduplicates and ranks primary evidence first", () => {
    ...
    response.output.push({
      ...
      action: { sources: [{ url: "https://pesacheck.org/not-cited" }] },
    });
    const items = extractSearchEvidence(response, "2026-09-12T12:00:00Z");
    expect(items.map((item) => item.source_type)).toEqual([
      "primary",
      "fact_check",
      "research",
    ]);
    ...
    expect(
      items.every((item) => item.published_at === null && item.hash === null),
    ).toBe(true);
  ...
  it("rejects incomplete, refused and empty provider output", () => {
    expect(() =>
      outputText({ ...message("Partial answer"), status: "incomplete" }),
    ).toThrow("Incomplete");
    expect(() =>
      outputText({
